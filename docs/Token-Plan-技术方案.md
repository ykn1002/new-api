...
6. **本期不做项**（依据方案「待确认问题」回复，详见覆盖分析第四节）：流量管控、套餐用户付费购买、退款、发票对账、调价回算存量、用户侧展示 Token。下文设计不含这些。
...
| GET | `/api/user/self` | 积分余额/累计消耗（含有效期、来源占比派生字段） | 改造（补字段） |
...
| 里程碑 | 范围 | 主要缺口 | 依赖 |
|---|---|---|---|
| **M1 配置基线** | 启用「积分」自定义货币、人民币≡美元汇率、两位小数核对、汇率权限收紧 | B-8/B-9/B-12 | 无 |
| **M2 积分批次账本** | `CreditBatch` 表 + `CreditUserQuota` 统一入口 + `SettleConsumeToBatches` 分摊 + 对账 + 老数据回填 | A-1/A-2 | M1 |
| **M3 有效期与流水** | 到期清零定时任务、顺延、`Log` 补 source/balance_after、过期类型、明细筛选 | A-1/B-10 | M2 |
| **M4 充值档位** | 档位结构扩展（名称/基础/赠送/有效期/上下架）+ 后台 CRUD + 到账拆批次 | A-3/B-3 | M2 |
...
| **M9 看板补全** | 总览聚合 + CSV 导出 | B-11/导出 | M2（积分聚合口径） |
| **M10 预警增强** | 百分比阈值 + 复用 `NotifyRootUser` 通知运营 | B-13 | M2 |
...
> 已澄清：看板「成本估算」= 消耗 Token 按售价折算的总金额（见 5.3），非上游真实成本，无需独立成本核算。
